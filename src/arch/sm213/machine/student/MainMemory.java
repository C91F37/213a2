...
public class MainMemory extends AbstractMainMemory {
  ...
  @Override protected boolean isAccessAligned (int address, int length) {
    // Accesses to main memory will be aligned
    // if the address is a multiple of the size of the object being tracked down
    // TODO
  ...
  @Override protected byte[] get (int address, int length) throws InvalidAddressException {
    // TODO
//    if (!isAccessAligned(address,length)) throw new InvalidAddressException();
    if (address+length-1 > mem.length) throw new InvalidAddressException();
    byte [] ret = new byte[length];
    System.arraycopy(mem, address, ret, 0, length);
    return ret;
  }
  ...
  @Override protected void set (int address, byte[] value) throws InvalidAddressException {
    // TODO
    if (address+value.length-1 > mem.length) throw new InvalidAddressException();
    int i = address;
    for (byte b : value) {
      mem[i] = b;
      i++;
